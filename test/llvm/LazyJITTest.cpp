...
// A host function that re-enters its owning VM to unregister a module while it
// is itself running inside a borrowed Executor::invoke. Taking the execution
// lock in unique mode on a thread that already holds it shared would
// self-deadlock, so teardown entry points must reject the reentrant call.
class HostReenterUnregister
    : public Runtime::HostFunction<HostReenterUnregister> {
public:
  HostReenterUnregister(VM::VM &Vm, Expect<void> &Out) : Owner(Vm), Out(Out) {}
  Expect<uint32_t> body(const Runtime::CallingFrame &, uint32_t A) {
    Out = Owner.unregisterModule("victim"sv);
    return A;
  }
...
class ReenterEnvModule : public Runtime::Instance::ModuleInstance {
public:
  ReenterEnvModule(VM::VM &Vm, Expect<void> &Out) : ModuleInstance("env") {
    addHostFunc("reenter", std::make_unique<HostReenterUnregister>(Vm, Out));
  }
};

// A borrowed executor (getExecutor / WasmEdge_VMGetExecutorContext) holds the
// execution lock in shared mode but not VM::Mutex, so a host function that
// mutates the same VM mid-invoke reaches the unique-mode acquisition on the
// same thread. It must be rejected with WrongVMWorkflow, not deadlock.
TEST_F(LazyJITTest, DirectInvokeRejectsReentrantTeardown) {
  auto VM = createLazyJITVM();

  // A second module for the host call to attempt to unregister.
  ASSERT_TRUE(VM->registerModule("victim", SimpleWasm));

  // Seeded with success so a host call that never runs would fail the test.
  Expect<void> TeardownResult = {};
  ReenterEnvModule HostMod(*VM, TeardownResult);
  ASSERT_TRUE(VM->registerModule(HostMod));

  auto *FuncInst = HostMod.findFuncExports("reenter"sv);
  ASSERT_NE(FuncInst, nullptr);

  std::vector<ValVariant> Params = {0U};
  std::vector<ValType> Types = {ValType(TypeCode::I32)};
  auto R = VM->getExecutor().invoke(FuncInst, Params, Types);

  // The invoke completes without self-deadlocking, and the reentrant teardown
  // was rejected rather than executed.
  ASSERT_TRUE(R);
  ASSERT_FALSE(TeardownResult);
  EXPECT_EQ(TeardownResult.error(), ErrCode::Value::WrongVMWorkflow);

  // The rejected call must not have torn down the victim module.
  EXPECT_TRUE(VM->unregisterModule("victim"sv));
}

// A host function that signals a waiting teardown thread, gives it time to take
// VM::Mutex and block on the execution lock, then re-enters its owning VM to
// unregister a module. This reproduces the cross-thread lock-order inversion:
// this invoke holds the execution lock shared, the teardown thread holds
// VM::Mutex and waits for the execution lock unique.
class HostReenterDuringConcurrentTeardown
    : public Runtime::HostFunction<HostReenterDuringConcurrentTeardown> {
public:
  HostReenterDuringConcurrentTeardown(VM::VM &Vm, std::atomic<bool> &Entered,
                                      Expect<void> &Out)
      : Owner(Vm), Entered(Entered), Out(Out) {}
  Expect<uint32_t> body(const Runtime::CallingFrame &, uint32_t A) {
...
class ConcurrentReenterEnvModule : public Runtime::Instance::ModuleInstance {
public:
  ConcurrentReenterEnvModule(VM::VM &Vm, std::atomic<bool> &Entered,
                             Expect<void> &Out)
      : ModuleInstance("env") {
    addHostFunc("reenter",
                std::make_unique<HostReenterDuringConcurrentTeardown>(Vm,
                                                                      Entered,
                                                                      Out));
  }
};

// Regression test for the VM::Mutex <-> execution-lock order inversion. Thread A
// runs a borrowed invoke holding the execution lock shared; thread B tears down
// a different module, holding VM::Mutex and blocking on the execution lock in
// unique mode. A's host function then re-enters to unregister a module. The
// reentrant teardown must be rejected before acquiring VM::Mutex; otherwise A
// waits on VM::Mutex (held by B) while B waits on the execution lock (held by
// A). If this regresses, the join below never returns and the test hangs.
TEST_F(LazyJITTest, ConcurrentReentrantTeardownDoesNotDeadlock) {
  auto VM = createLazyJITVM();

  ASSERT_TRUE(VM->registerModule("victim", SimpleWasm));
  ASSERT_TRUE(VM->registerModule("other", SimpleWasm));

  std::atomic<bool> Entered{false};
  Expect<void> TeardownResult = {};
  ConcurrentReenterEnvModule HostMod(*VM, Entered, TeardownResult);
  ASSERT_TRUE(VM->registerModule(HostMod));

  auto *FuncInst = HostMod.findFuncExports("reenter"sv);
  ASSERT_NE(FuncInst, nullptr);

  // Seeded with failure so a teardown thread that never runs fails the test.
  Expect<void> OtherResult = Unexpect(ErrCode::Value::WrongVMWorkflow);
  std::thread Teardown([&]() {
    while (!Entered.load(std::memory_order_acquire)) {
      std::this_thread::yield();
    }
    // Takes VM::Mutex, then blocks on the execution lock (held shared by the
    // in-flight invoke) until that invoke returns.
    OtherResult = VM->unregisterModule("other"sv);
  });
  ...
  // The reentrant teardown was rejected, so "victim" remains and can still be
  // unregistered here; the concurrent teardown of "other" on the other thread
  // completed successfully.
  EXPECT_TRUE(VM->unregisterModule("victim"sv));
  EXPECT_TRUE(OtherResult);
}

// A host function that signals a waiting teardown thread, lets it take the
// execution lock unique and block, then re-enters its owning VM through a READ
// entry point (getFunctionList). Under the former two-lock hierarchy the read
// took VM::Mutex while the teardown thread held VM::Mutex and waited for the
// execution lock held by this invoke — an inverted lock order that deadlocked.
// With a single lock the reentrant read reuses the lock this invoke already
// holds instead of re-acquiring it.
class HostReenterReadDuringConcurrentTeardown
    : public Runtime::HostFunction<HostReenterReadDuringConcurrentTeardown> {
public:
  HostReenterReadDuringConcurrentTeardown(VM::VM &Vm, std::atomic<bool> &Entered,
                                          std::atomic<bool> &ReadDone)
      : Owner(Vm), Entered(Entered), ReadDone(ReadDone) {}
  Expect<uint32_t> body(const Runtime::CallingFrame &, uint32_t A) {
    Entered.store(true, std::memory_order_release);
    std::this_thread::sleep_for(std::chrono::milliseconds(100));
    auto List = Owner.getFunctionList();
...
class ReadReenterEnvModule : public Runtime::Instance::ModuleInstance {
public:
  ReadReenterEnvModule(VM::VM &Vm, std::atomic<bool> &Entered,
                       std::atomic<bool> &ReadDone)
      : ModuleInstance("env") {
    addHostFunc("reenter",
                std::make_unique<HostReenterReadDuringConcurrentTeardown>(
                    Vm, Entered, ReadDone));
  }
};

// Regression test for the reentrant-read deadlock (the read-path analog of
// ConcurrentReentrantTeardownDoesNotDeadlock). Thread A runs a borrowed invoke
// holding the execution lock shared; thread B tears down a different module,
// taking the lock unique and waiting for A to drain. A's host function then
// re-enters the VM through getFunctionList(). The reentrant read must reuse the
// lock A already holds; otherwise A blocks on the lock (pending-unique for B)
// while B blocks on A, and the join below never returns.
TEST_F(LazyJITTest, ConcurrentReentrantReadDoesNotDeadlock) {
  auto VM = createLazyJITVM();
  ...
  ReadReenterEnvModule HostMod(*VM, Entered, ReadDone);
  ASSERT_TRUE(VM->registerModule(HostMod));
  ...
  std::thread Teardown([&]() {
    ...
    // Takes the execution lock unique (waiting for the in-flight invoke to
    // drain), then unregisters "other".
    OtherResult = VM->unregisterModule("other"sv);
  });
  ...
  ASSERT_TRUE(R);
  // The reentrant read completed instead of deadlocking, and the concurrent
  // teardown of "other" then succeeded once the invoke drained.
  EXPECT_TRUE(ReadDone.load(std::memory_order_acquire));
  EXPECT_TRUE(OtherResult);
  EXPECT_TRUE(VM->unregisterModule("victim"sv));
}

// A benign test-owned host function: returns its argument and touches no VM
// state. Because the test owns the module, its FunctionInstance stays valid
// across VM teardown, so a borrowed invoke can keep running it while cleanup()
// runs on another thread.
class BenignHostFunc : public Runtime::HostFunction<BenignHostFunc> {
public:
  Expect<uint32_t> body(const Runtime::CallingFrame &, uint32_t A) { return A; }
};

class BenignEnvModule : public Runtime::Instance::ModuleInstance {
public:
  BenignEnvModule() : ModuleInstance("env") {
    addHostFunc("work", std::make_unique<BenignHostFunc>());
  }
};

// Concurrency stress test: a borrowed executor invoke runs in a tight loop on
// one thread while cleanup() tears the VM down and rebuilds it on another. Time
// measuring is enabled so the invoke records into the VM-shared Statistics that
// cleanup() also resets; Statistics is internally synchronized (an atomic
// instruction count and a Timer with its own mutex), so that access needs no
// extra serialization. The execution lock instead serializes module-instance
// teardown against the in-flight invoke. This guards against a deadlock, crash,
// or use-after-free in that interleaving and doubles as a ThreadSanitizer
// scaffold (run the suite under -fsanitize=thread).
TEST_F(LazyJITTest, ConcurrentInvokeDuringCleanupNoDataRace) {
  Configure Conf;
  Conf.getRuntimeConfigure().setRunMode(RunMode::LazyJIT);
  Conf.getCompilerConfigure().setOptimizationLevel(
      CompilerConfigure::OptimizationLevel::O1);
  ...
  BenignEnvModule HostMod;
  ASSERT_TRUE(VM->registerModule(HostMod));
  auto *FuncInst = HostMod.findFuncExports("work"sv);
  ASSERT_NE(FuncInst, nullptr);
  ...
  std::thread Invoker([&]() {
    while (!Stop.load(std::memory_order_acquire)) {
      VM->getExecutor().invoke(FuncInst, Params, Types);
      Invokes.fetch_add(1, std::memory_order_relaxed);
    }
  });

  // Let the invoker get going, then run cleanup() concurrently several times to
  // widen the window for any unsynchronized Stat access.
  while (Invokes.load(std::memory_order_relaxed) < 50) {
    std::this_thread::yield();
  }
  for (int I = 0; I < 200; ++I) {
    VM->cleanup();
  ...
  // The invoker ran concurrently with cleanup() without crashing or deadlocking;
  // under ThreadSanitizer it additionally surfaces any data race the interleaving
  // actually exposes.
  EXPECT_GT(Invokes.load(std::memory_order_relaxed), 0U);
}
