...
Expect<int32_t> AVFormatOpenInput::body(const Runtime::CallingFrame &Frame,
  ...
  std::copy_n(UrlId.data(), UrlSize, std::back_inserter(TargetUrl));
...
Expect<int32_t> AVDumpFormat::body(const Runtime::CallingFrame &Frame,
  ...
  std::copy_n(UrlBuf.data(), UrlSize, std::back_inserter(TargetUrl));
  FFMPEG_PTR_FETCH(AvFormatCtx, AvFormatCtxId, AVFormatContext);
...
Expect<int32_t> AVFormatAllocOutputContext2::body(
  ...
  if (FormatLen > 0) {
    ...
    std::copy_n(FormatId.data(), FormatLen, std::back_inserter(Format));
  }
  MEM_PTR_CHECK(AvFormatCtxId, MemInst, uint32_t, AvFormatCtxPtr,
                "Failed when accessing the return AVFormatContext Memory"sv);

  std::string File;
  std::copy_n(FileId.data(), FileNameLen, std::back_inserter(File));
...
Expect<int32_t> AVIOOpen2::body(const Runtime::CallingFrame &Frame,
  ...
  std::string TargetUrl;
  std::copy_n(UrlId.data(), UrlLen, std::back_inserter(TargetUrl));
...
Expect<uint32_t> AVGuessCodec::body(const Runtime::CallingFrame &Frame,
  ...
  std::string FileName;
  std::string MimeType;
  std::copy_n(ShortNameBuf.data(), ShortNameLen, std::back_inserter(ShortName));
  std::copy_n(FileNameBuf.data(), FileNameLen, std::back_inserter(FileName));
  std::copy_n(MimeTypeBuf.data(), MimeTypeLen, std::back_inserter(MimeType));
