...
public:
  ...
  /// Hook run when a module is registered, instantiated, or re-instantiated,
  /// to prepare per-mode state before execution. \p PreAllocated, when
  /// non-null, is a heap-allocated copy the caller already made; strategies
  /// that store the module can adopt it to avoid a second deep copy.
  virtual Expect<void>
  onModulePrepared(AST::Module &Module,
